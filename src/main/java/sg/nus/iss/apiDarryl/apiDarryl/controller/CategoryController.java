...
@Tag(description="Category resources that provides access to available category data", name = "Category Resource")
@RestController
@RequestMapping("/api")
public class CategoryController {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "${api.response-code.notFound.desc}", 
            content = {
                @Content(examples = {@ExampleObject(value="")})
            }
        )
    ...
    public ResponseEntity<Category> saveCategory(@RequestBody Category category){
        try{
            Category categoryEnt = categoryService.saveCategory(category);

            return new ResponseEntity<>(categoryEnt, HttpStatus.CREATED);
        }
        catch(Exception e){
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseEntity<Category> updateCategory(@RequestBody Category category){
        try{
            Category savedCategory = categoryService.updCategory(category, category.getId());
